...
const Game = (_ => {
  ...
  const $hangman = document.querySelector(".hangman");
    ...
    const init = _ => {
      ...
      //2. built out our guessing word to render
      guessingWord = Array(chosenWord.length).fill("_");
      guesses = [];
      lives = 7;
      //3. show initial page or screen
      showInitPage();
      listeners();
      Board.init();
    }
    ...
    const check =  guess => {
      ...
      // check if the guess exists in choosenword
...
//1. choose a random word chosenWord = chosenWord(); //apple
